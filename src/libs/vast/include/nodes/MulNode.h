    ...
    public:
        ...
        bool equals(const VAST& other) const override {
            if (const MulNode* m1 = dynamic_cast<const MulNode*>(&other)) {
                return ((this->getLeft()->equals(*m1->getLeft()) && this->getRight()->equals(*m1->getRight())) ||
                    (this->getLeft()->equals(*m1->getRight()) && this->getRight()->equals(*m1->getLeft()))
                );
            }
            return false;
        }
        ...
        std::string getString() const override {
            std::unique_ptr<VAST> left = this->getLeft()->clone();
            std::unique_ptr<VAST> right = this->getRight()->clone();
            if (is_var(*left.get()) || is_var(*right.get())) {
